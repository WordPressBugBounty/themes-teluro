...
// admin notice
(function ($) {
    var processBuilderInstalationStepts = function () {
            ...
            return _regenerator2.default.wrap(function _callee$(_context) {
                while (1) {
                    switch (_context.prev = _context.next) {
                        ...
                        case 8:
                            wp.ajax.post('teluro_front_set_predesign', {
                                index: selectedFrontPage,
                                nonce: teluro_builder_status.teluro_front_set_predesign_nonce
                            });
                            if (teluro_builder_status.status === "not-installed") {
                                toggleProcessing(true);
                                installBuilder(callback);
                            }

                            if (teluro_builder_status.status === "installed") {
                                toggleProcessing(true);
        ...
        return function processBuilderInstalationStepts(_x) {
            return _ref.apply(this, arguments);
        };
    }();

    var $notice_container = $('.teluro-admin-big-notice--container');
    ...
    function disableNotice() {
        wp.ajax.post('teluro_disable_big_notice', {
            nonce: teluro_builder_status.teluro_disable_big_notice_nonce
        });
    }

    function toggleProcessing(value) {
        $(window).on('beforeunload.teluro-admin-big-notice', function () {
            return true;
        });
        if (value) {
            $('.teluro-admin-big-notice').addClass('processing');
            $('.teluro-admin-big-notice .action-buttons').fadeOut();
        } else {
            $('.teluro-admin-big-notice').removeClass('processing');
        }
    }
    ...
    function installBuilder(callback) {
        pluginNotice(teluro_builder_status.messages.installing);
        $.get(teluro_builder_status.install_url).always(function () {
            toggleProcessing(true);
            activateBuilder(callback);
        }).always(function () {
            $(window).off('beforeunload.teluro-admin-big-notice');
        });
    }

    function activateBuilder(callback) {
        pluginNotice(teluro_builder_status.messages.activating);

        wp.ajax.post('teluro_activate_plugin', {
            slug: teluro_builder_status.slug,
            source: 'notice-' + selectedFrontPage,
            _wpnonce: teluro_builder_status.colibri_plugin_install_activate_nonce
        }).done(function () {
            $(window).off('beforeunload.teluro-admin-big-notice');
            if (callback) {
                callback();
    ...
    $notice_container.on('click', '.view-all-demos', function () {
        ...
        processBuilderInstalationStepts(function () {
            window.location = teluro_builder_status.view_demos_url;
        });
    });
    ...
    var teluroInstallPluginSuccess = function teluroInstallPluginSuccess(response) {
    ...
    var teluroInstallPlugin = function teluroInstallPlugin(event) {
        ...
        wp.updates.installPlugin({
            ...
            success: teluroInstallPluginSuccess
        });
    };

    var teluroActivatePlugin = function teluroActivatePlugin(event) {
        var $button = $(event.target);
    ...
    $document.on("click", ".colibri-plugin.install-now", teluroInstallPlugin);
    $document.on("click", ".colibri-plugin.activate-now", teluroActivatePlugin);
